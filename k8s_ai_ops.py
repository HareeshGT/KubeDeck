...
def _general_question_response(request: str, namespace: str = None):
    ...
    if any(phrase in clean for phrase in identity_patterns):
        return (
            "I'm the Kubernetes Ops Mind assistant in Deckhand. "
...
class K8sAIOpsWidget(QWidget):
    ...
    def _build_ui(self):
        # Keep the real Ops Mind UI inside a separate content widget so the
        ...
        description = QLabel(
            "Describe a Kubernetes operation in plain English. Previous Ops Mind "
    ...
    def showEvent(self, event):
        ...
        # Re-check whenever Ops Mind becomes visible so it unlocks without a restart.
        self.refresh_ai_access()
    ...
    def _start_voice_recording(self):
        ...
        self.voice_btn.setToolTip(
            "Stop recording and send the transcription to Ops Mind"
        )
